...
class Athlete:
    def __init__(
        self, id, full_name, display_name, short_name, links, jersey, headshot, position_abbreviation, team_id, active
    ):
        self.id = id
...
class ESPNScoreboard:
    def __init__(self, sport_name, league_name):
        ...
        if sport_name == "football":
            ...
            if league_name not in ["nba", "wnba", "womens-college-basketball", "mens-college-basketball"]:
                raise ValueError(
                    "League name should be one of the following: [nba, wnba, womens-college-basketball, mens-college-basketball]"
                )
        ...
        self.sport_name = sport_name
        self.league_name = league_name
        self.SCOREBOARD_ENDPOINT = (
            f"http://site.api.espn.com/apis/site/v2/sports/{self.sport_name}/{self.league_name}/scoreboard"
        )
    ...
    def _parse_competition_geo_broadcasts(self, competition_geo_broadcasts_arr):
        ...
        for geo_broadcast in competition_geo_broadcasts_arr:
            ...
            geo_broadcasts.append(
                GeoBroadcast(type_id, type_short_name, market_id, market_type, media_short_name, lang, region)
            )
    ...
    def _parse_athlete(self, athlete_obj):
        ...
        return Athlete(
            id, full_name, display_name, short_name, links, jersey, headshot, position_abbreviation, team_id, active
        )
    ...
    def get_scoreboard(self, date):
        ...
        if "events" in parsed_json and isinstance(parsed_json["events"], list):
            for event in parsed_json["events"]:
                ...
                events.append(
                    Event(
                        id, uid, date, name, short_name, season_year, season_type, competitions, links, weather, status
                    )
                )
